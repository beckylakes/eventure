...
const EventDetails = async (props: SearchParamProps) => {
  ...
  const isEventCreator = !tmEvent && event?.organizer._id === userId;
  ...
  const eventImage = tmEvent ? imageHD.url : event.imageUrl;
  ...
  const eventLocation = tmEvent
    ? tmEvent._embedded.venues[0].city.name
    : event.location;
  ...
  const eventOrganizer = tmEvent
    ...
    const relatedEvents =
    (await getRelatedEventsByCategory({
      categoryName: event
        ? event.category.name
  ...
  const relatedTMEvents =
    (await getAllTMEvents({
      query: "",
      limit: 3,
      page: 1,
      category: tmEvent
  ...
  const allRelatedEvents = [...(relatedEvents?.data || []), ...(relatedTMEvents?.data || [])];
              ...
              {event && !tmEvent ? (
                isEventCreator ? (
                  <Button
                    ...
                    className="button rounded-full px-6 py-3 text-lg font-semibold"
                  >
                    <Link href={`/events/${event._id}/update`}>Edit Event</Link>
                  </Button>
                ) : !hasBoughtTicket ? (
                  ...
                  >
                    <Link href="/profile">You're going! View Orders?</Link>
                  </Button>
                )
              ) : null}
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="p-regular-20 flex items-center gap-3">
                <Image
                  ...
                  height={32}
                />
                <p className="p-medium-16 lg:p-regular-20">{eventLocation}</p>
              </div>
            </div>
